...
void empaquetar(unsigned char b[], unsigned char valor[] , unsigned char valor2[]){
	...
	b[1] |=  valor[0] ;//preguntar como limitar el tamanio del int guardo los 8 bits en valor
	b[2] |= valor[1] ;
	
	b[3] |= valor2[0] ; //preguntar como limitar el tamanio del int guardo los 8 bits en valor
		b[4] |= valor2[1] ;
...
//XXXXXXXX			0000XXXX
  ...
  /* For example: for(;;) { } */
	...
	AD1_MeasureChan(TRUE,0); //12 bits
	...
	empaquetar(b,valor,valor2);
	 ...
	 if(D1_GetVal()){
			b[1] |= 0b00010000; //asigno el bit del canal digital al b[1]
		}else b[1] |= 0b00000000;
		if(D2_GetVal()){
				b[3] |= 0b00010000; //asigno el bit del canal digital al b[3]
			} else b[3] |= 0b00000000;
	//b[1]= 0b00001001;
	//b[2]= 0b11011101;
	// Envio de la data
